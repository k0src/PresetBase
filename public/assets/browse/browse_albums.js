...
/* SORTING */
const sortAlbums = function (key) {
  window.location.href = `/browse/albums?sort=${encodeURIComponent(key)}`;
};
...
const filterList = function (query) {
  ...
  document.querySelectorAll(".result-entry").forEach((entry) => {
    ...
    const matches =
      !lowerQuery || artist.includes(lowerQuery) || album.includes(lowerQuery);
