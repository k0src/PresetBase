...
class UserRoleColumn extends Components.DBTableColumn {
  constructor() {
    super("is_admin", "Is Admin", "user-entry--role user--role");
  }
...
class UserTableDataManager extends Components.DBTableDataManager {
...
class UsersViewManager extends Components.DBTableViewManager {
  constructor(listContainer) {
    super(listContainer, UserTableDataManager);
  }

  get columns() {
    return [
      new Components.DBTableColumn(
        "username",
        "Username",
        "user-entry--primary user--username"
      ),
      new Components.DBTableColumn(
        "email",
        "Email",
        "user-entry--primary user--email"
      ),
      new Components.DBTableColumn(
        "timestamp",
        "Join Date",
        "user-entry--date user--timestamp"
      ),
      new Components.DBTableColumn(
        "authenticated_with",
        "Auth Provider",
        "user-entry--auth user--auth"
      ),
      new UserRoleColumn(),
    ];
  }
  ...
  get headerClassName() {
    return "result-columns grid-layout--users";
  }

  get entryClassName() {
    return "grid-layout--users user-entry";
  }

  get rowNumberClassName() {
    return "user-entry--number";
  }
  ...
  createActionButton(row) {
    ...
    button.innerHTML = '<i class="fa-solid fa-pen-to-square edit--icon"></i>';
...
class UsersResultsManager extends Components.DBTableResultsManager {
  get stateKey() {
    return "manage-users";
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const viewManager = new UsersViewManager(
    domManager.getElement("listContainer")
  );

  const resultsManager = new UsersResultsManager(viewManager, domManager);
