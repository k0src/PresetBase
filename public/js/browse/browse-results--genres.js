...
sortSelect.addEventListener("change", () => {
  localStorage.setItem("genreSortSelected", sortSelect.value);

  window.location.href = `/browse/genres?sort=${encodeURIComponent(
    sortSelect.value
  )}`;
});
...
viewModeToggle.addEventListener("click", () => {
  ...
  localStorage.setItem("userGenresViewMode", mode);
});

/* ------------------------- Setting values on load ------------------------- */
const setSortSelectValue = function () {
  const sortValue = localStorage.getItem("genreSortSelected");
  if (sortValue) {
...
const setUserView = function () {
  const userView = localStorage.getItem("userGenresViewMode") || "list";
...
const filterList = function (query) {
  ...
  document.querySelectorAll(".result-entry").forEach((entry) => {
    ...
    const matches = !lowerQuery || genre.includes(lowerQuery);
